...
FORMAT_NO_RUMAH = "no"
FORMAT_TANGGAL_LAHIR = "%d-%m-%Y"
FORMAT_TANGGAL_DIBUAT = "%S:%M:%H %d-%m-%Y"
...
def cek_data(
    ...
    try:
        ...
        temp_tanggal_lahir = temp_tempat_tanggal_lahir[1]
        ...
        try:
            temp_tanggal_lahir = datetime.strptime(
                temp_tanggal_lahir, FORMAT_TANGGAL_LAHIR)
            temp_tanggal_lahir = pytz.timezone(
                TIME_ZONE).localize(temp_tanggal_lahir)
        except:
            print(
                "tanggal lahir error: format tanggal lahir salah"
                + "(error saat convert ke datetime object)")
            raise Exception(
                "Mohon isi tanggal lahir sesuai format yang dicontohkan")
        # Memastikan tanggal lahir sebelum tanggal sekarang
        tanggal_sekarang = datetime.now(pytz.timezone(TIME_ZONE))
        if temp_tanggal_lahir > tanggal_sekarang:
            print(
                "tempat tanggal lahir error: tanggal lahir terlalu muda")
            raise Exception("Anda terlalu muda !")
...
def tempel_data(letak_tempel, values):
    ...
    for temp_row, i in enumerate(values):
        for temp_column, j in enumerate(i):
            ...
            # Mengganti format tanggal lahir
            elif temp_column == 3:
                print(j)
                temp_label = Label(
                    letak_tempel, text=date.strftime(
                        j, FORMAT_TANGGAL_LAHIR))
            # Mengganti format tanggal dibuat
            elif temp_column == 11:
                temp_label = Label(
                    letak_tempel, text=datetime.strftime(
                        j, FORMAT_TANGGAL_DIBUAT),
                    font=font(10))
            # Data lainnya
            else:
...
conn = mysql.connector.connect(
    host='host mysql mu',
    user='username mysql mu',
    passwd='p@**w0rd mysql mu',
    database='database mysql mu')
# Create cursor
curs = conn.cursor()

# Create table
# Table yang digunakan untuk menyimpan data
'''
